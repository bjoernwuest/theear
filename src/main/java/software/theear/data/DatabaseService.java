...
@Service public class DatabaseService {
  ...
  private final BlockingQueue<Runnable> m_DatabaseTransactionsQueue = new LinkedBlockingQueue<>();
  private final ExecutorService m_DatabaseTransactionExecutor = new ThreadPoolExecutor(4, 8, 0L, TimeUnit.MILLISECONDS, m_DatabaseTransactionsQueue);
  
  public DatabaseService(@Autowired DataConfiguration DBConfig) {
...
/* EXANPLE HOW TO REGISTER EXTENSIONS      ResultSet rSet = stmt.executeQuery("SELECT extname FROM pg_extension WHERE extname = 'uuid-ossp'");
   ...
   */
  public <T> Future<T> scheduleTransaction(@Nonnull final Function<Connection, T> Transaction) { return this.scheduleTransaction(Transaction, null, null); }
   ...
   * @return A {@link Future} on the transaction which can be queried, waited on, etc. and which will hold the result of the transaction.
   */
  public <T> Future<T> scheduleTransaction(@Nonnull final Function<Connection, T> Transaction, @Nonnull BiConsumer<Function<Connection, T>, T> OnDone) { return this.scheduleTransaction(Transaction, OnDone, null); }
   ...
   * @return A {@link Future} on the transaction which can be queried, waited on, etc. and which will hold the result of the transaction.
   */
  public <T> Future<T> scheduleTransaction(@Nonnull final Function<Connection, T> Transaction, @Nonnull BiFunction<Function<Connection, T>, Exception, T> OnError) { return this.scheduleTransaction(Transaction, null, OnError); }
   ...
   * @return A {@link Future} on the transaction which can be queried, waited on, etc. and which will hold the result of the transaction.
   */
  public <T> Future<T> scheduleTransaction(@Nonnull final Function<Connection, T> Transaction, BiConsumer<Function<Connection, T>, T> OnDone, BiFunction<Function<Connection, T>, Exception, T> OnError) {
    return m_DatabaseTransactionExecutor.submit(new Callable<T>() {
      @Override public T call() throws Exception {
        try (Connection conn = m_DatabasePool.getConnection()) {
          try {
